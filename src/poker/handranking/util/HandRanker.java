...
/**
 * Handranking utiliy tools.
 * Includes preflop / postflop hand evaluation functions plus probability weight mapping.
...
public class HandRanker
{
    ...
    private static final Map<Integer, Integer> cardRankToStartIndex = new HashMap<Integer, Integer>() {{
      put(Card.RANK_2, 0); put(Card.RANK_3, 1); put(Card.RANK_4, 2);
      put(Card.RANK_5, 3); put(Card.RANK_6, 4); put(Card.RANK_7, 5);
      put(Card.RANK_8, 6); put(Card.RANK_9, 7); put(Card.RANK_10, 8);
      put(Card.RANK_JACK, 9); put(Card.RANK_QUEEN, 10); 
      put(Card.RANK_KING, 11); put(Card.RANK_ACE, 12);
    }};  
    ...
    private static final Map<Integer, Integer> cardSuitToStartIndex = new HashMap<Integer, Integer>() {{
      put(Card.SUIT_CLUBS, 0); put(Card.SUIT_DIAMONDS, 1); 
      put(Card.SUIT_HEARTS, 2); put(Card.SUIT_SPADES, 3);
    }};  
    ...
    /**
     * This function maps a two card combination to an index inside a
     * 13x13 input List. The two dimensional input List should
     * represent a mapping for 169 unique starting hands
     * (for example, starting hand probabilties / field array).
    ...
    /**
     * This function defines a ranking for indiviudal cards.
     * The resulting value is used to define a natural ordering for individual cards.
     * For suits, the ordering is defined as follows: clubs < diamonds < hearts < spades
     * @param c The card
    ...
    /**
     * Create a field array that corresponds to uniform probabilities,
     * for the 169 unique holecard combinations. All values are set to 1.
    ...
    /**
     * This function perform the hand ranking for preflop hands, 
     * given the holecards and the number of active opponents as input.
     * The returned value corresponds to the "income rate" of the particular
     * starting hand (View Loki paper for more info).
     * @param c1 The first holecard
     * @param c2 The second holecard
    ...
    /**
    * Postflop Handvalue calculation for a 5-card combination.
    * All possible subcombinations (kickers etc..) are taken into account during
    * the mapping. Hence, every unique 5 card combination will be mapped
    * to a different value: The stronger the combination the higher the returned
    * value will be.
    ...
    */
    ...
    /**
    * Check for quads. 
    * Note: Assumes that cards are presorted by rank.
    ...
    */
    ...
    /**
    * Check for flush (can also be straight flush)
    * Note: Assumes that cards are presorted by suit
    * @param cards The 5-card combination to evaluate.
    ...
    */
    ...
    /**
    * Check for straight (can also be straight flush)
    * Note: Assumes that cards are presorted by rank.
    * @param cards The 5-card combination to evaluate.
    ...
    */
    ...
    /**
     * Check for set (can also be full house or four of a kind)
     * Note: Assumes that cards are presorted by rank.
     * @param cards The 5-card combination to evaluate.
    ...
    /**
     * Check for two pair (can also be full house)
     * Note: Assumes that cards are sorted by rank.
     * @param cards The 5-card combination to evaluate.
    ...
    /**
     * Check for pair (can also be two pair, set, full house or quads)
     * Note: Assumes that cards are sorted by rank.
     * @param cards The 5-card combination to evaluate.
    ...
    /**
     * Return straight flush hand value. Assume sorted by rank.
     * STRAIGHT_FLUSH + kicker
    ...
    /**
     * Return full house hand value. Assume sorted by rank.
     * FULL_HOUSE + SETCard*10000 + PAIRCard
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
    ...
    /**
     * Return flush hand value. Assume sorted by rank.
     * FLUSH + valueCards
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
    ...
    /**
     * Return straight hand value. Assume sorted by rank.
     * STRAIGHT + high card
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
    ...
    /**
     * Return quads hand value. Assume sorted by rank.
     * STRAIGHT + QUADCard*10000 + kicker
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
    ...
    /**
     * Return set hand value. Assume sorted by rank.
     * SET + SetCard * 10000 + kicker1*100 + kicker2
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
    ...
    /**
     * Return two pair hand value. Assume sorted by rank.
     * TWO_PAIRS + HighPair * 10000 + LowPair*100 + kicker
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
    ...
    /**
     * Return one pair hand value. Assume sorted by rank.
     * ONE_PAIR + Pair*10000 + kicker1*500 + kicker2*15 + kicker3
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
    ...
    /**
     * Return basic card value. Assume sorted by rank.
     * 15^4 highcard1 + 15^3 highcard2 + ... + lowCard
     * @param cards The 5-card combination to evaluate.
     * @return hand value
     */
